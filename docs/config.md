...
| `api.http.auth_token` | `string` | Optional token for sensitive HTTP API endpoints (currently `query_config` and `apply_config`). Supports `Authorization: Bearer <token>` or `X-Zenfeed-Auth-Token`. |                               | Recommended            |
| `api.http.disable_cors` | `bool` | If `true`, cross-origin browser requests are rejected and CORS headers are not returned (same-origin requests still work). | `false`                       | No                     |
| `api.http.allowed_origins` | `list of strings` | CORS allowlist when `disable_cors` is `false`. Requests with `Origin` not in this list are rejected. | `["http://localhost:1400", "http://127.0.0.1:1400"]` | No |
...
| `scrape.sources[].max_items_per_scrape` | `int` | Optional maximum number of new feeds to process from this source on each scrape. Useful for development/testing. `0` means no limit. | `0` | No |
...
| `scrape.sources[].rss.detail` | `object` | Optional detail content configuration. When configured, Zenfeed resolves a detail source from each item link after de-duplication and stores the fetched detail content in `podcast_source`. Exactly one of `detail.rss` or `detail.crawl` must be configured. | `nil` | No |
| `scrape.sources[].rss.detail.link_regex` | `string` | Optional regular expression used to match the feed `link` and extract named parameters. Named capture groups can be referenced by `detail.rss.rsshub_route_path_template` or `detail.crawl.url_template`. Required when `detail.rss` is set; optional when `detail.crawl` is set, in which case omitting it crawls the original `link` directly. | | Yes (if `detail.rss` is set) |
| `scrape.sources[].rss.detail.rss` | `object` | Detail configuration for fetching content via RSSHub. Mutually exclusive with `detail.crawl`. | `nil` | Yes (if `detail.crawl` is not set) |
| `scrape.sources[].rss.detail.rss.rsshub_route_path_template` | `string` | RSSHub route path template for the detail feed. Example: `v2ex/post/{{ .postid }}` or `zhihu/question/{{ .questionId }}`. | | Yes (if `detail.rss` is set) |
| `scrape.sources[].rss.detail.crawl` | `object` | Detail configuration for fetching content by crawling the page. Mutually exclusive with `detail.rss`. | `nil` | Yes (unless `detail.rss` is set) |
| `scrape.sources[].rss.detail.crawl.type` | `string` | Crawl type. Supported values: `crawl` (local crawl, default) and `crawl_by_jina` (via Jina Reader). | `crawl` | No |
| `scrape.sources[].rss.detail.crawl.url_template` | `string` | Optional crawl URL template. If omitted, the original feed `link` is crawled directly. Named capture groups from `link_regex` can be referenced here; if `link_regex` is omitted, only the built-in `{{ .link }}` variable is available. | | No |
